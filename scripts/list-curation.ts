...
for (const r of rows.sort((a,b)=> b.stars - a.stars)) {
  ...
  const f = r.featured ? "★" : " ";
  const h = r.hidden ? "h" : " ";
  console.log(`${r.name.padEnd(32)}${String(r.stars).padStart(3)}  ${r.lang.padEnd(10)}  ${v} ${f} ${h}`);
}
...
if ((curation.featured as string[]).some(n=> !projects.some(p=>p.name===n))) {
  console.log("\n⚠ featured contains unknown names:", (curation.featured as string[]).filter(n=> !projects.some(p=>p.name===n)));
}
if (hidden.size===0) console.log("\nTip: add low-value repos to hidden: test, dotfiles, Mvc3QuizQuestions, ...");
